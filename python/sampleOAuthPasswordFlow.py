...
#
"""
Sample API client using OAuth 2 password flow.  It will will repeat
...
def authenticate(url, client_id, username):
    """
...
def refresh_tokens(url, client_id, username, refresh_token):
    """
    ...
    try:
        ...
        # refresh_token may have expired.  Authenticate to get new tokens
        return authenticate(url, client_id, username)
    tokens = json.load(response)
    access_token = tokens["access_token"]
    try:
        new_refresh_token = tokens["refresh_token"]
        print("Retrieved new refresh token...")
...
# Copy your EDG API user name and paste it here
...
while True:
    ...
    try:
        ...
        if error.code == 401:
            ...
            access_token, refresh_token = refresh_tokens(auth_server_url, client_id, username, refresh_token)
